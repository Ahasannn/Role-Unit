...
class LengthEstimatorRecord:
    ...
    strategy_name: str
    output_length: int
    prompt_tokens: int = 0  # token count of the full prompt_base (numerical feature)
    item_id: str = ""  # grouping key for train/val/test split
...
class LengthEstimatorConfig:
    ...
    dropout: float = 0.1
    log_transform: bool = False  # train on log1p(tokens), expm1 at inference
...
class LengthEstimator(nn.Module):
    """MLP that predicts output token count.

    Input: semantic_emb(384->64) + prompt_tokens(1, BatchNorm'd)
           + strategy_emb(16) + role_emb(16) + model_emb(16) = 113-dim
    Output: Softplus(linear(32,1))  (non-negative token count, or log1p thereof)
    """

    NUM_NUMERICAL = 1  # prompt_token_count
    ...
    def __init__(
        ...
        *,
        ...
        self.numerical_norm = nn.BatchNorm1d(self.NUM_NUMERICAL)
        ...
        fused_dim = config.semantic_compress_dim + self.NUM_NUMERICAL + 3 * config.embedding_dim
    ...
    def forward(
        ...
        if prompt_token_counts.dim() == 1:
            prompt_token_counts = prompt_token_counts.unsqueeze(-1)  # (B,) -> (B,1)
        ...
        num_features = self.numerical_norm(prompt_token_counts.float())
        fused = torch.cat(
            [
                semantic_features,
                num_features,
...
class LengthEstimatorDataset(Dataset):
    def __init__(
        ...
        self.prompt_token_counts = torch.tensor(
            [float(record.prompt_tokens) for record in self.records],
            dtype=torch.float32,
        )
        ...
        self.targets = torch.log1p(raw_targets) if log_transform else raw_targets
...
class LengthEstimatorBundle:
    ...
    config: Optional[LengthEstimatorConfig] = None
    ...
    def predict_length(
        ...
        *,
        ...
        role_name: str,
        strategy_name: str,
        prompt_token_count: float = 0.0,
    ) -> float:
        """Encode prompt text and predict output length. Prefer predict_length_from_embedding() to avoid redundant encoding."""
        device = next(self.model.parameters()).device
        semantic = self.encoder([prompt])[0].to(device)
        return self.predict_length_from_embedding(
            query_embedding=semantic,
    ...
    def predict_length_from_embedding(
        self,
        ...
        *,
        ...
        """Predict output length from a pre-computed query embedding (no SentenceTransformer call)."""
        device = next(self.model.parameters()).device
        semantic = query_embedding.to(device)
        ...
        ptc = torch.tensor([prompt_token_count], device=device, dtype=torch.float32)
        self.model.eval()
        with torch.no_grad():
            prediction = self.model(semantic.unsqueeze(0), ptc, strategy_id, role_id, model_id)
        return self._inverse_transform(float(prediction.item()))
    ...
    def predict_length_from_ids(
        ...
        *,
        ...
        self.model.eval()
        with torch.no_grad():
            prediction = self.model(semantic.unsqueeze(0), ptc, strategy_id_tensor, role_id_tensor, model_id_tensor)
        return self._inverse_transform(float(prediction.item()))
    ...
    with path.open("r", encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            if output_length is None or output_length < min_length:
                continue
            prompt_tokens = _safe_int(row.get(prompt_tokens_field)) or 0
    ...
    *,
    ...
    for _ in range(epochs):
        ...
        for semantic, ptc, strategy_id, role_id, model_id, targets in loader:
            ...
            optimizer.zero_grad()
            predictions = model(semantic, ptc, strategy_id, role_id, model_id)
